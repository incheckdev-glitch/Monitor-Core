-- InCheck360 production concurrency + note-log compatibility hardening
--
-- Exposed by the expanded live E2E matrix:
--   * parallel agreement->invoice conversions could calculate the same SA/YYYY/NN
--     value and collide on invoices_invoice_id_key;
--   * the generic migrated-resource list path sorts by updated_at, while the two
--     append-only CRM note-log tables historically only had created_at.
--
-- Safe to re-run. No existing business records are renumbered.

begin;

-- --------------------------------------------------------------------------
-- 1) Generic note-log resource compatibility
-- --------------------------------------------------------------------------

alter table if exists public.lead_note_logs
  add column if not exists updated_at timestamptz;

update public.lead_note_logs
...
alter table if exists public.deal_note_logs
  add column if not exists updated_at timestamptz;

update public.deal_note_logs
set updated_at = coalesce(updated_at, created_at, now())
where updated_at is null;
...
-- --------------------------------------------------------------------------
-- 2) Deal conversion: serialize count-based DEAL/YYYY/NNNN allocation
-- --------------------------------------------------------------------------
...
begin
  ...
  -- Prevent concurrent conversions from reading the same count before either insert commits.
  perform pg_advisory_xact_lock(hashtext('incheck360:deal-business-id'), extract(year from current_date)::integer);

  select 'DEAL/' || to_char(current_date, 'YYYY') || '/' || lpad((count(*) + 1)::text, 4, '0')
  into v_code
  from public.deals;
...
-- --------------------------------------------------------------------------
-- 3) Proposal conversion: serialize Proposal#NNNNN allocation
-- --------------------------------------------------------------------------
...
begin
  ...
  select 'Proposal#' || lpad((count(*) + 1)::text, 5, '0')
  into v_id
...
-- --------------------------------------------------------------------------
-- 4) Agreement -> invoice: serialize client and invoice business IDs
-- --------------------------------------------------------------------------
...
begin
  ...
  if c is null then
    ...
    -- Re-check after acquiring the lock in case another transaction created it.
    select cl.id into c
    from public.clients cl
    where cl.source_agreement_id = a.id
    limit 1;
    ...
    if c is null then
      ...
      values(
        'CLIENT/' || to_char(current_date, 'YYYY') || '/' || lpad((select count(*) + 1 from public.clients)::text, 4, '0'),
  ...
  -- This is the race that the parallel production E2E exposed.
  perform pg_advisory_xact_lock(hashtext('incheck360:invoice-business-id'), extract(year from current_date)::integer);

  select 'SA/' || to_char(current_date, 'YYYY') || '/' || lpad((count(*) + 1)::text, 2, '0')
  into v_no
...
-- --------------------------------------------------------------------------
-- 5) Invoice -> receipt: serialize RV/YYYY/NN allocation
-- --------------------------------------------------------------------------
...
begin
  ...
  select 'RV/' || to_char(current_date, 'YYYY') || '/' || lpad((count(*) + 1)::text, 2, '0')
  into v_no
