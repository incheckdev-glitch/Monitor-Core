...
function applyEconomyUi(){
  const count=document.getElementById('liCount');
  if(count){
    const current=Math.min(MAX_COUNT,Math.max(1,Number(count.value)||DEFAULT_COUNT));
    count.innerHTML='<option value="1">1 — Minimum</option><option value="2">2</option><option value="3">3 — Lowest usage</option><option value="5">5 — Maximum</option>';
    count.value=String([1,2,3,5].includes(current)?current:DEFAULT_COUNT);
  }
  const hint=document.getElementById('liGenerateHint');
  if(hint&&!active){
    hint.textContent='Economy mode · GPT-5.6 Luna · max 3 web searches · up to 5 prospects. Identical searches within 24h are reused.';
  }
}
...
async function poll(id,resumed=false){
  ...
  let wait=3000;
  busy(true,resumed?'Resuming existing research — no duplicate OpenAI charge.':'Economy research is running. Results will appear automatically.');
  try{
    while(active===id){
      const p=await call({action:'status',run_id:id}),s=txt(p.status).toLowerCase();
      if(s==='completed'){
        active='';error('');
        busy(false,`${Number(p.result_count||0)} suggestions are ready · economy mode.`);
        await refresh(id);
        toast(`${Number(p.result_count||0)} suggested leads ready.`);
        return;
      }
      const sec=Math.max(1,Math.round((Date.now()-started)/1000));
      busy(true,`${s==='queued'||s==='starting'?'Preparing':'Researching'} · economy mode · ${sec}s elapsed.`);
...
async function start(){
  ...
  try{
    ...
    if(p.cached){
      active='';
      busy(false,`${Number(p.result_count||0)} suggestions loaded from the last identical search · $0 new OpenAI research.`);
...
async function resume(){
  ...
  try{
    ...
    const r=await supabase.from('lead_intelligence_runs')
      .select('id,status,openai_response_id,started_at')
