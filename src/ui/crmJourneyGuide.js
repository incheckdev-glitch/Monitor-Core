const CrmJourneyGuide = (() => {
  const S = { bound:false, observer:null, timer:null, rulesRegistered:false };
  ...
  const STAGES = [
    ...
    { key:'contacts', label:'Contact', icon:'2' },
    { key:'leads', label:'Lead', icon:'3' },
    { key:'deals', label:'Deal', icon:'4' },
    { key:'proposals', label:'Proposal', icon:'5' }
  ];
  ...
  function stage(key=currentModule()){ return STAGES.find(x => x.key === key) || null; }
  ...
  function journeySignature(activeKey){ return `${activeKey}|${STAGES.map(s => canOpen(s.key) ? '1' : '0').join('')}`; }
  function journeyMarkup(activeKey, signature){
    const g = GUIDANCE[activeKey] || GUIDANCE.company;
    return `<section class="crm-journey-card" data-crm-journey data-crm-signature="${esc(signature)}">
      ...
      <div class="crm-journey-rail" role="list" aria-label="CRM lifecycle">
        ${STAGES.map((s,i) => {
          const allowed = canOpen(s.key), active = s.key === activeKey;
          return `<button type="button" role="listitem" class="crm-journey-stage ${active?'is-active':''}" data-crm-journey-nav="${esc(s.key)}" ${allowed?'':'disabled'}><span>${s.icon}</span><b>${esc(s.label)}</b>${i < STAGES.length-1 ? '<i aria-hidden="true">›</i>' : ''}</button>`;
        }).join('')}
      </div>
  ...
  function ensureJourney(view, key){
    ...
    const signature = journeySignature(key);
    let card = view.querySelector('[data-crm-journey]');
    if (card?.dataset.crmSignature === signature) return;
    if (!card) {
      const wrap = document.createElement('div'); wrap.innerHTML = journeyMarkup(key, signature); card = wrap.firstElementChild;
      ...
      if (header && view.contains(header)) header.insertAdjacentElement('afterend', card); else view.prepend(card);
    } else {
      const wrap = document.createElement('div'); wrap.innerHTML = journeyMarkup(key, signature); card.replaceWith(wrap.firstElementChild);
    }
  }
  ...
  function formContext(form){
    ...
    if (id === 'companyForm') {
      ...
      return { step:'1 · Company', title:'Create / Edit Company', text:'The company is the CRM master record. Save it once, then reuse it for contacts, leads and deals.', items:[
    ...
    if (id === 'contactForm') {
      const companies = selectedCount('contactCompanyInput'), first = value('contactFirstNameInput'), last = value('contactLastNameInput');
      return { step:'2 · Contact', title:'Create / Edit Contact', text:'A contact must belong to at least one company and must have enough identity to avoid duplicate people.', items:[
    ...
    if (id === 'leadForm') {
      ...
      const qualified = norm(value('leadFormStatus')) === 'qualified', convertAfterSave = form.dataset.convertAfterSave === 'true';
      return { step:'3 · Lead', title:convertAfterSave?'Lead Conversion Readiness':'Create / Edit Lead', text:convertAfterSave?'This lead is being prepared for conversion. Save the required note/workflow fields first; the ERP will then continue the conversion flow.':'Keep the company/contact links intact and record the next action before saving.', items:[
    ...
    if (id === 'dealForm') {
      const leadEl = document.getElementById('dealFormLeadId');
      const lead = value('dealFormLeadId') || clean(leadEl?.dataset?.leadUuid || '');
      const directCreate = (form.dataset.mode || 'create') !== 'edit' && !lead;
      const company = value('dealFormCompanySelector') || value('dealFormCompanyId'), contact = value('dealFormContactSelector') || value('dealFormContactId');
      return { step:'4 · Deal', title:'Deal / Proposal Readiness', text:directCreate?'This is a direct Deal create, so Company and Contact must be selected explicitly.':'This Deal is linked to a source Lead. Keep those links intact while progressing the opportunity.', items:[
        ...
        {label:'Next Follow-up', ok:Boolean(value('dealNextFollowUpAtInput')), note:'Mandatory for every deal change.'},
        {label:'New Note', ok:Boolean(value('dealFormNotes')), note:'Required by the current Deal save workflow.'},
        {label:'Qualified stage for Proposal', ok:norm(value('dealFormStage')) === 'qualified', note:'Required before Create Proposal.'}
      ]};
    }
    return null;
  }
  function formSignature(ctx){ return JSON.stringify([ctx.step,ctx.title,ctx.text,...ctx.items.flatMap(x=>[x.label,Boolean(x.ok),x.note])]); }
  function ensureFormCard(form){
    const ctx = formContext(form); if (!ctx) return;
    const signature = formSignature(ctx);
    let card = form.querySelector(':scope > .crm-form-journey-card');
    if (card?.dataset.crmSignature === signature) return;
    if (!card) {
      card = document.createElement('section'); card.className = 'crm-form-journey-card';
    ...
    card.dataset.crmSignature = signature;
  ...
  function ensureWorkflowPanelCard(){
    const body = document.querySelector('.workflow-guide-root:not([hidden]) .workflow-guide-panel-body'), key = currentModule();
    if (!body) return;
    let card = body.querySelector('.crm-journey-workflow-card');
    if (!GUIDANCE[key]) { card?.remove(); return; }
    const signature = `panel|${key}`;
    if (card?.dataset.crmSignature === signature) return;
  ...
  function registerFormRules(){
    if (S.rulesRegistered) return true;
    const fg = window.FormGuide; if (!fg?.registerRules) return false;
    fg.registerRules('companyForm',{required:['companyNameInput'],notes:{companyNameInput:'Company Name is required by the Company save validation.'}});
    fg.registerRules('contactForm',{required:['contactCompanyInput'],notes:{contactCompanyInput:'At least one company is required by the Contact save validation.',contactFirstNameInput:'First Name or Last Name: at least one must be provided.',contactLastNameInput:'First Name or Last Name: at least one must be provided.'}});
    fg.registerRules('leadForm',{
      ...
      notes:{leadFormCompanyName:'A company must be selected from the CRM list before saving.',leadFormContactName:'A contact must be selected from the CRM list before saving.',leadFormNotes:'A new note is required when creating/editing a lead and before conversion.'}
    });
    fg.registerRules('dealForm',{
      required:['dealNextFollowUpAtInput','dealFormNotes'],
      conditional:{
        dealFormCompanySelector:{when:({root})=>(root?.dataset?.mode||'create')!=='edit'&&!clean(document.getElementById('dealFormLeadId')?.value||document.getElementById('dealFormLeadId')?.dataset?.leadUuid),reason:'Company is mandatory when creating a Deal directly without a source Lead.',optionalReason:'Company is inherited/linked for this Deal flow.'},
        dealFormContactSelector:{when:({root})=>(root?.dataset?.mode||'create')!=='edit'&&!clean(document.getElementById('dealFormLeadId')?.value||document.getElementById('dealFormLeadId')?.dataset?.leadUuid),reason:'Contact is mandatory when creating a Deal directly without a source Lead.',optionalReason:'Contact is inherited/linked for this Deal flow.'}
      },
      notes:{dealNextFollowUpAtInput:'Next Follow-up is required for every deal change.',dealFormNotes:'A new note is required by the current Deal save workflow.'}
    });
    S.rulesRegistered = true;
    fg.refresh?.();
    return true;
  }
  ...
  function scan(){
    ...
    if (GUIDANCE[key]) {
      const tab = tabFor(key), view = document.getElementById(tab?.getAttribute('aria-controls') || '') || document.querySelector('.view.active,[role="tabpanel"].active');
      if (view) ensureJourney(view,key);
    }
    ['companyForm','contactForm','leadForm','dealForm'].forEach(id=>{const form=document.getElementById(id);if(form&&visible(form))ensureFormCard(form);});
  ...
  function bind(){
    ...
    document.addEventListener('click',e=>{
      const nav=e.target?.closest?.('[data-crm-journey-nav]'); if(nav){e.preventDefault();navigate(nav.dataset.crmJourneyNav);return;}
