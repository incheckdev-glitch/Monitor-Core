const FormGuide = (() => {
  ...
  function labelText(el,root){
    const l=labelFor(el,root);
    if(l){
      const clone=l.cloneNode(true); clone.querySelectorAll('.form-guide-field-badge,input,select,textarea,button').forEach(n=>n.remove());
      return clean(clone.textContent).replace(/\*+$/,'').trim();
    }
  ...
  function controls(root){
    return qsa(root,'input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]),select,textarea,[contenteditable="true"]').filter(visible);
  }
  function nativeContract(root){
  ...
  function classify(el,root){
    ...
    if(nativeContract(root)) return {kind:'optional',reason:'Not marked required by the exposed form definition. Workflow validation remains authoritative.'};
    return {kind:'conditional',reason:'No explicit required/optional rule is exposed by this form. The workflow or legacy validation may make it mandatory.'};
  }
  ...
  function bind(){
    ...
    if(typeof MutationObserver!=='undefined'){
      ...
      S.observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style','hidden','aria-hidden','disabled','readonly','required','aria-required']});
    }
    schedule(0);
  }
