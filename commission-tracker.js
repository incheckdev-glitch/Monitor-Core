(function initSalesCommissionTracker(global){
  ...
  const MODULE = {
    ...
    async remove(id){
      if(!this.canDelete())return;const row=this.state.commissions.find(item=>String(item.id)===String(id));if(!row)return;const stats=this.statsFor(row);if(stats.paid>0){this.toast('A commission with paid installments cannot be deleted. Undo the payments first.');return;}if(!confirm(`Delete commission tracking for ${row.invoice_number}?`))return;const {data,error}=await this.db().rpc('delete_sales_commission',{p_commission_id:id});if(error){this.toast(error.message||'Unable to delete commission record.');return;}if(data!==true){this.toast('Commission record was not deleted.');return;}this.toast('Commission record deleted.');await this.refresh();
    },
    exportCsv(){
