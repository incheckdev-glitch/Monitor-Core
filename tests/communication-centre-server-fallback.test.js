...
assert.doesNotMatch(endpoint, /SUPABASE_SERVICE_ROLE_KEY/, 'server fallback must not depend on a privileged service-role key');
assert.match(endpoint, /auth\.getUser\(token\)/, 'server fallback must verify the caller access token');
assert.match(endpoint, /global: \{ headers: \{ Authorization: `Bearer \$\{token\}` \} \}/, 'fallback writes must execute as the authenticated Supabase user');
...
assert.match(endpoint, /insertConversationWithRetry\(userClient, \{/, 'conversation writes must remain subject to user RLS');
const conversationInsert = endpoint.match(/insertConversationWithRetry\(userClient, \{([\s\S]*?)\n    \}\);/)?.[1] || '';
...
assert.match(endpoint, /userClient\.from\('communication_centre_participants'\)\.insert\(participantRows\)/, 'participant writes must remain subject to user RLS');
...
assert.match(endpoint, /cleanupPartial\(userClient, conversationId\)/, 'partial conversation data must be rolled back through the authenticated client');
...
assert.match(client, /fetch\('\/api\/communication-centre-create'/, 'missing create RPC must use the authenticated Vercel endpoint');
...
console.log('Communication Centre authenticated RLS fallback checks passed.');
